...
open struct TSwiftHelper {
    ...
    open static var appBundleID: String? {
        return Bundle.main.bundleIdentifier
    }

    // MARK: Returns both app's version and build numbers "v0.3(7)"
    open static var appVersionAndBuild: String? {
    ...
    open static var isRelease: Bool {
        #if DEBUG
        return false
        #else
        return true
    ...
    open static var isInTestFlight: Bool {
    ...
    open static var topMostVC: UIViewController? {
    ...
    /// 
    open static var horizontalSizeClass: UIUserInterfaceSizeClass {
        return self.topMostVC?.traitCollection.horizontalSizeClass ?? UIUserInterfaceSizeClass.unspecified
    }

    /// 
    open static var verticalSizeClass: UIUserInterfaceSizeClass {
    ...
    open static var screenStatusBarHeight: CGFloat {
        return UIApplication.shared.statusBarFrame.height
    }

    // MARK: Return screen's height without StatusBar
    open static var screenHeightWithoutStatusBar: CGFloat {
        if screenOrientation == .portrait || screenOrientation == .portraitUpsideDown {
    ...
    open static var currentRegion: String? {
    ...
    open static func detectScreenShot(_ action: @escaping () -> Void) {
    ...
    open static func iterateEnum<T: Hashable>(_: T.Type) -> AnyIterator<T> {
        var i = 0
        return AnyIterator {
    ...
    open static func dispatchDelay(_ second: Double, closure:@escaping () -> Void) {
    ...
    open static func runThisAfterDelay(seconds: Double, after: @escaping () -> Void) {
    ...
    open static func runThisAfterDelay(seconds: Double, queue: DispatchQueue, after: @escaping () -> Void) {
        let time = DispatchTime.now() + Double(Int64(seconds * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
        queue.asyncAfter(deadline: time, execute: after)
    }

    // MARK: Submits a block for asynchronous execution on the main queue
    open static func runThisInMainThread(_ block: @escaping () -> Void) {
        DispatchQueue.main.async(execute: block)
    }

    // MARK: Runs in Default priority queue
    open static func runThisInBackground(_ block: @escaping () -> Void) {
        DispatchQueue.global(qos: .default).async(execute: block)
    }

    // MARK: Runs every second, to cancel use: timer.invalidate()
    @discardableResult open static func runThisEvery(
    ...
    open var globalMainQueue: DispatchQueue {
    ...
    open static func requestImage(_ url: String, success: @escaping (UIImage?) -> Void) {
    ...
    open static func requestJSON(_ url: String, success: @escaping ((Any?) -> Void), error: ((NSError) -> Void)?) {
        requestURL(url,
