...
extension CGPoint {
    ...
    open init(vector: CGVector) {
        self.init(x: vector.dx, y: vector.dy)
    }
    
    // MARK: Constructor from CGFloat
    open init(angle: CGFloat) {
        self.init(x: cos(angle), y: sin(angle))
    }

    // MARK: Adds two CGPoints.
    open static func + (this: CGPoint, that: CGPoint) -> CGPoint {
        return CGPoint(x: this.x + that.x, y: this.y + that.y)
    }

    // MARK: Subtracts two CGPoints.
    open static func - (left: CGPoint, right: CGPoint) -> CGPoint {
        return CGPoint(x: left.x - right.x, y: left.y - right.y)
    }

    // MARK: Multiplies a CGPoint with a scalar CGFloat.
    open static func * (point: CGPoint, scalar: CGFloat) -> CGPoint {
    ...
    open static func dotProduct(this: CGPoint, that: CGPoint) -> CGFloat {
        return this.x * that.x + this.y * that.y
    }

    // MARK: Performs a linear interpolation between two CGPoint values.
    open static func linearInterpolation(startPoint: CGPoint, endPoint: CGPoint, interpolationParam: CGFloat) -> CGPoint {
